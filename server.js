...
body { font-family: -apple-system, BlinkMacSystemFont, 'SF Pro Display', sans-serif; background: var(--bg); color: var(--text); font-size: 15px; }
...
.screen-chat { display: none; flex-direction: column; padding: 14px; }
.screen-chat.active { display: flex; }
 
.nav { display: flex; background: var(--surface); border-top: 0.5px solid var(--border); flex-shrink: 0; }
.nav-btn { flex: 1; display: flex; flex-direction: column; align-items: center; gap: 3px; padding: 8px 4px 10px; border: none; background: transparent; color: var(--text3); font-size: 10px; font-weight: 600; cursor: pointer; transition: color 0.15s; }
...
.start-btn { width: 100%; padding: 15px; background: var(--primary); color: white; border: none; border-radius: var(--radius); font-size: 15px; font-weight: 700; cursor: pointer; box-shadow: 0 4px 16px rgba(13,158,117,0.3); transition: all 0.15s; display: flex; align-items: center; justify-content: center; gap: 8px; }
...
.send-btn { width: 36px; height: 36px; border-radius: 50%; background: var(--primary); border: none; cursor: pointer; display: flex; align-items: center; justify-content: center; flex-shrink: 0; transition: all 0.15s; }
...
.mic-btn { flex: 1; padding: 11px; border: 1.5px solid var(--border); border-radius: var(--radius); background: var(--surface); color: var(--text2); font-size: 13px; font-weight: 600; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 6px; transition: all 0.15s; }
...
const TOPICS = [
  { id:'places', e:'🗺️', name:'Amazing Places', lv:'B1 · Descriptive',
    sys:\`You are a warm English speaking coach for Kazakh college students at Pre-Intermediate level. Topic: Amazing Places.
After each student message:
1. Reply naturally and encouragingly (1-2 sentences)
2. Then add feedback in EXACTLY this format on a new line:
[FB]F:X|V:X|G:X|GOOD:text|TIP:text[/FB]
Where X is score 1-5. GOOD = one thing done well. TIP = one improvement.
Start the conversation with: "Hello! I'm your AI speaking coach 😊 Let's talk about amazing places! If you could visit any place in the world, where would you go and why?"\` },
  { id:'culture', e:'🌍', name:'Culture Shock', lv:'B1 · Argumentative',
    sys:\`You are a warm English speaking coach for Kazakh college students at Pre-Intermediate level. Topic: Culture Shock.
After each student message:
1. Reply naturally and encouragingly (1-2 sentences)
2. Then add feedback in EXACTLY this format on a new line:
[FB]F:X|V:X|G:X|GOOD:text|TIP:text[/FB]
Where X is score 1-5. GOOD = one thing done well. TIP = one improvement.
Start with: "Hello! Great to see you 😊 Today we talk about culture. Have you ever experienced something from another culture that really surprised you?"\` },
  { id:'cities', e:'🏙️', name:'Cities & Countries', lv:'B1 · Comparative',
    sys:\`You are a warm English speaking coach for Kazakh college students at Pre-Intermediate level. Topic: Cities and Countries.
After each student message:
1. Reply naturally and encouragingly (1-2 sentences)
2. Then add feedback in EXACTLY this format on a new line:
[FB]F:X|V:X|G:X|GOOD:text|TIP:text[/FB]
Where X is score 1-5. GOOD = one thing done well. TIP = one improvement.
Start with: "Hi there! Ready to practice English? 😊 Let's compare cities. Which city would you most love to live in — and why?"\` },
  { id:'animals', e:'🦁', name:'Animal World', lv:'B1 · Discussion',
    sys:\`You are a warm English speaking coach for Kazakh college students at Pre-Intermediate level. Topic: Animal World.
After each student message:
1. Reply naturally and encouragingly (1-2 sentences)
2. Then add feedback in EXACTLY this format on a new line:
[FB]F:X|V:X|G:X|GOOD:text|TIP:text[/FB]
Where X is score 1-5. GOOD = one thing done well. TIP = one improvement.
Start with: "Hello! Welcome to speaking practice 😊 Today's topic is animals! What is your favourite animal and what makes it special?"\` },
];
...
function goTab(name,idx){
  ...
  if(name==='chat'){el.style.display='flex';}else{el.style.display='block';}
...
function startChat(){
  ...
  curSess={topic:sel.name,e:sel.e,scores:[],msgs:0,date:new Date().toLocaleDateString('kk-KZ'),saved:false};
  goTab('chat',1);
...
async function callAI(msgs){
  ...
  try{
    const res=await fetch('/chat',{
      ...
      body:JSON.stringify({system:sel.sys,messages:msgs})
    });
    const d=await res.json();
    const txt=d.content?.map(b=>b.text||'').join('')||'Great answer! Keep going — tell me more about that.';
    hideDots();
    const p=parseFB(txt);
    ...
    hideDots();
    addBubble('ai',"Hello! I'm your AI speaking coach 😊 Let's start practicing! Tell me something about yourself in English.");
  }
  busy=false;
...
function showDots(){
  ...
  w.appendChild(dotEl); w.scrollTop=w.scrollHeight;
}
...
function toggleMic(){
  if(!('webkitSpeechRecognition' in window||'SpeechRecognition' in window)){
    alert('Браузер дауысты тану қолдамайды. Chrome қолданыңыз.');
    return;
  }
  if(recording){
